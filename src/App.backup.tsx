...
export default function App() {
  ...
  // Carrega dados automaticamente ao trocar de aba
  useEffect(() => {
    if (isLoggedIn && loggedUser?.codigo_igreja) {
      if (activeTab === 'membros') fetchMembers(loggedUser.codigo_igreja);
      if (activeTab === 'financeiro') fetchFinanceiro(loggedUser.codigo_igreja);
      if (activeTab === 'agenda') fetchAgenda(loggedUser.codigo_igreja);
    }
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
              <div className="flex justify-between items-center border-b pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Caixa e Bancos</p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y text-xs text-slate-700">
                    {filteredLancamentos.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-4 text-center text-slate-400">Nenhum lançamento encontrado.</td>
                      </tr>
                    ) : (
          ...
          <div className="bg-white max-w-md w-full rounded-2xl shadow-2xl border border-slate-200 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveConta} className="p-6 space-y-4">
              <div>
                ...
                <input type="text" required placeholder="Ex: 002, 003..." value={contaForm.codigo_conta} onChange={(e) => setContaForm({ ...contaForm, codigo_conta: e.target.value })} className="w-full rounded-lg border p-2 text-sm font-mono" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Nome da Conta / Banco *</label>
                <input type="text" required placeholder="Ex: Sicoob, Itaú..." value={contaForm.nome_conta} onChange={(e) => setContaForm({ ...contaForm, nome_conta: e.target.value })} className="w-full rounded-lg border p-2 text-sm" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
            ...
            <form onSubmit={handleSaveCompromisso} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required placeholder="Ex: Pastor, Membro..." value={agendaForm.dono_codigo} onChange={(e) => setAgendaForm({ ...agendaForm, dono_codigo: e.target.value })} className="w-full rounded-lg border p-2 text-sm" />
                </div>
                <div>
